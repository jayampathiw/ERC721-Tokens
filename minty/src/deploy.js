...
async function deployContract(name, symbol) {
  ...
  const address = await minty.getAddress();
  console.log(
    `deployed contract for token ${name} (${symbol}) to ${address} (network: ${network})`
  );

  return deploymentInfo(hardhat, minty, address);
}

function deploymentInfo(hardhat, minty, address) {
  return {
    network: hardhat.network.name,
    contract: {
      name: CONTRACT_NAME,
      address: address,
      signerAddress: minty.runner.address,
